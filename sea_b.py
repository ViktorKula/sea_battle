...
class BoardOutException(BoardException):
    def __str__(self):
        return 'Координаты за пределами игрового поля'


class BoardUsedException(BoardException):
    def __str__(self):
        return 'Ячейка уже использована'
...
class Board:
    def __init__(self, hid=False, size=6):
        ...
        self.hid = hid
        self.count = 0
        self.field = [['O'] * size for _i in range(size)]
        self.busy = []
        self.ships = []
...
class User(Player):
    def ask(self):
        while True:
            cords = input('Ваш ход (формат:x y): ').split()
            if len(cords) != 2:
